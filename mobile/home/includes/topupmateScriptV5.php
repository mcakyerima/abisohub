<script>
    $("document").ready(function(){
        ...
        $("#rpinForm").submit(function(e){
            e.preventDefault();

            if($("#rnew-pin").val() != $("#confirm-pin").val()){
                swal("Error!","New Pin & Confirm Pin Don't Match.","error");
                return 0;
            }

            if($("#rnew-pin").val().length < 4){$(this).val(null); swal("Opps!!","Pin Length Should Be Four Digits.","info"); return;}
            if($("#confirm-pin").val().length < 4){$(this).val(null); swal("Opps!!","Pin Length Should Be Four Digits.","info"); return;}

            $('#reset-pin-btn').removeClass("gradient-highlight");
            $('#reset-pin-btn').addClass("btn-secondary");
            $('#reset-pin-btn').html('<i class="fa fa-spinner fa-spin" aria-hidden="true"></i> Updating ...');
            
            
            $.ajax({
                url:'home/includes/route.php?reset-pin',
                data: new FormData($(this)[0]),
                cache: false,
                contentType: false,
                processData: false,
                ...
                success:function(resp){
                    ...
                    $('#reset-pin-btn').removeClass("btn-secondary");
                    $('#reset-pin-btn').addClass("gradient-highlight");
                    $('#reset-pin-btn').html("Reset Pin");
       ...
       $("#electricityForm").submit(function(e){
        ...
        $("#networkid").on("change",function(){
            ...
            let share = $('#networkid').find(":selected").attr('share');
            ...
            let thegroup='<option value="">Select Type</option>';
                ...
                //Check If Network Is Disabled
                if(share=="On"){
                   thegroup+='<option value="Share">Data Share</option>';
                }
